...
impl<Data: Send> Sender<Data> for MpscSender<Data> {
    fn send(&mut self, msg: Data) -> impl Future<Output = Status> + Send {
        async {
...
impl<Data: Send> Receiver<Data> for MpscReceiver<Data> {
    fn recv(&mut self) -> impl Future<Output = Result<Data>> + Send {
        async {
...
impl<'a, Data: Send + 'a, SI: SeqId + Send + 'a> TestMaker<Data, SI> {
